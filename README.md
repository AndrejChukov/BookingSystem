...
| `GET`  | `/api/users` | Получение информации о текущей сессии | `USER`, `WORKER`, `ADMIN` |
...
| `POST` | `/api/rooms` | Добавление новой комнаты *(Сброс кеша)* | `ADMIN` |
| `PUT`  | `/api/rooms/{id}` | Обновление характеристик комнаты *(Сброс кеша)*| `WORKER`, `ADMIN` |
| `DELETE`| `/api/rooms/{id}` | Удаление комнаты *(Сброс кеша)* | `ADMIN` |

### 💻 Оборудование (Equipment)
| HTTP Метод | Эндпоинт | Описание | Роль / Доступ |
| :--- | :--- | :--- | :--- |
| `GET`  | `/api/equipment` | Список всего инвентаря | `WORKER`, `ADMIN` |
